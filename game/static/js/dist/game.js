...
class AcGameObject {
    ...
    last_update() { // 一帧的最后执行一次
    }

    on_destroy() { // 在销毁之前会执行一次
    }
...
class MultiPlayerSocket {
    constructor(playground) {
        ...
        this.ws = new WebSocket("wss://app6051.acapp.acwing.com.cn/wss/multiplayer/");
...
class AcGamePlayground {
   ...
   create_uuid() {
    ...
    hide() { // 关闭playground界面
        ...
        this.$playground.empty(); // 清空playground html
...
`);
    ...
    add_listening_events_login() { // 添加登录事件监听
        let outer = this;
    ...
    add_listening_events_register() { // 添加注册事件监听
        let outer = this;
